...
namespace websockets
{
    ...
    WebsocketsClient::WebsocketsClient(std::shared_ptr<network::TcpClient> client) : _client(client),
                                                                                     _endpoint(client),
    ...
    WebsocketsClient::WebsocketsClient(const WebsocketsClient &other) : _client(other._client),
...
#ifdef ESP8266
        if (
            this->_optional_ssl_fingerprint || (this->_optional_ssl_rsa_cert && this->_optional_ssl_rsa_private_key) || (this->_optional_ssl_ec_cert && this->_optional_ssl_ec_private_key) || this->_optional_ssl_trust_anchors || this->_optional_ssl_known_key)
        {
    ...
    bool areKeysEqualIgnoringCase(const WSString &key1, const WSString &key2)
    {
    ...
    bool WebsocketsClient::setHeader(const WSInterfaceString key, const WSInterfaceString value, bool AddIfNotExist)
    {
        WSString Key = internals::fromInterfaceString(key);
        WSString Value = internals::fromInterfaceString(value);

        for (auto &header : _customHeaders)
        {
            if (areKeysEqualIgnoringCase(header.first, Key)) // Ignore case of header
            {
                header.second = Value;
                return true; // Header found and updated
            }
        }
        ...
        if (AddIfNotExist)
        {
            addHeader(key, value); // Add missing header
            return true;
        }
        return false;
    }
